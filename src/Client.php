...
namespace Specialist\Smpp;

use Exception;
use Specialist\Smpp\exceptions\SmppException;
use Specialist\Smpp\exceptions\SmppTransportException;
use Specialist\Smpp\transport\Stream;
use Specialist\Smpp\helpers\GsmEncoderHelper;
...
class Client
{
	 ...
	 */
	public function sendSMS (Address $from, Address $to, string $message, ?array $tags = null, int $dataCoding = SMPP::DATA_CODING_DEFAULT, int $priority = 0x00, ?string $scheduleDeliveryTime = null, ?string $validityPeriod = null): string
	{
		...
		if ($doCsms)
		{
			if (self::$csmsMethod === self::CSMS_PAYLOAD)
			{
				...
				return $this->submit_sm($from, $to, null, (empty($tags) ? [$payload] : array_merge($tags, $payload)), $dataCoding, $priority, $scheduleDeliveryTime, $validityPeriod);
			}
			elseif (self::$csmsMethod === self::CSMS_8BIT_UDH)
			{
				...
				foreach ($parts as $part)
				{
					...
					$res = $this->submit_sm($from, $to, $udh . $part, $tags, $dataCoding, $priority, $scheduleDeliveryTime, $validityPeriod, (self::$smsEsmClass | 0x40));
					$seqnum ++;
				}
				return $res;
			}
	 ...
	 * @throws Exception
	 */
	protected function submit_sm (Address $source, Address $destination, ?string $short_message = null, ?array $tags = null, int $dataCoding = SMPP::DATA_CODING_DEFAULT, int $priority = 0x00, ?string $scheduleDeliveryTime = null, ?string $validityPeriod = null, ?string $esmClass = null): string
	{
		if (is_null($esmClass))
			$esmClass = self::$smsEsmClass;
		
		// Construct PDU with mandatory fields
		$pdu = pack('a1cca' . (strlen($source->value ?? '') + 1) . 'cca' . (strlen($destination->value ?? '') + 1) . 'ccc' . ($scheduleDeliveryTime ? 'a16x' : 'a1') . ($validityPeriod ? 'a16x' : 'a1') . 'ccccca' . (strlen($short_message) + (self::$smsNullTerminateOctetStrings ? 1 : 0)), self::$smsServiceType, $source->ton, $source->npi, $source->value, $destination->ton, $destination->npi, $destination->value, $esmClass, self::$smsProtocolID, $priority, $scheduleDeliveryTime, $validityPeriod, self::$smsRegisteredDeliveryFlag, self::$smsReplaceIfPresentFlag, $dataCoding, self::$smsSmDefaultMessageID, strlen($short_message), $short_message);
	 ...
	 * @return array
	 */
	protected function splitMessageString (string $message, int $split, int $dataCoding = SMPP::DATA_CODING_DEFAULT): array
	{
		switch ($dataCoding)
		{
			case SMPP::DATA_CODING_DEFAULT:
	 ...
	 */
	protected function bind (string $login, string $pass, int $commandID): mixed
	{
		// Make PDU body
	 ...
	 * @throws Exception
	 */
	public function enquireLink (): mixed
	{
     ...
     * @param string|null $pduBody - PDU body
     * @return bool|Pdu
     * @throws Exception
     */
	...
	protected function sendPDU (Pdu $pdu): void
    {
		...
		$this->debugLog('Send PDU          : %d bytes', $length);
		$this->debugLog('  %s', chunk_split(bin2hex($header . $pdu->body), 2, ' '));
		$this->debugLog('  command_id      : 0x%s', dechex($pdu->id));
		$this->debugLog('  sequence number : %d', $pdu->sequence);
	 ...
	 * @throws SmppException
	 */
	protected function readPduResponse (int $sequenceNumber, int $commandID)
	{
		...
		{
			$pdu = $this->pduQueue[$i];
			if (($pdu->sequence == $sequenceNumber && ($pdu->id === $commandID || $pdu->id === SMPP::GENERIC_NACK)) || ($pdu->sequence === null && $pdu->id === SMPP::GENERIC_NACK))
			{
		...
		do
		{
			...
			if ($pdu)
			{
				if ($pdu->sequence === $sequenceNumber && ($pdu->id === $commandID || $pdu->id === SMPP::GENERIC_NACK))
					return $pdu;
	...
	protected function readPDU ()
	{
		...
		$this->debugLog('Read PDU          : %d bytes', $length);
		$this->debugLog('  %s', chunk_split(bin2hex($bufLength . $bufHeaders . $body), 2, ' '));
		$this->debugLog('  command id      : 0x%s', dechex($command_id));
		$this->debugLog('  command status  : 0x%s %s', dechex($command_status), SMPP::getStatusMessage($command_status));
		$this->debugLog('  sequence number : %d', $sequence_number);
	...
	protected function parseTag (array & $ar)
	{
		...
		$this->debugLog('Parsed tag:');
		$this->debugLog('  id     : 0x%s', dechex($tag->id));
		$this->debugLog('  length : %d', $tag->length);
		$this->debugLog('  value  : %s', chunk_split(bin2hex($tag->value), 2, ' '));
